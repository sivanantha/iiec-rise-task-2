...
def register(username,pwd):
	cursor.execute("SELECT * FROM cred WHERE username= ?",(username,))
	user=cursor.fetchall()
	if user == []:
		cursor.execute("INSERT INTO cred(username,password) VALUES(?, ?)",(username, pwd,))
		connection.commit()
		connection.close()
		return 1
	else:
...
def forgot_password(username,pwd):
	...
	if user == []:
		...
		cursor.execute("UPDATE cred SET password= ? WHERE username= ?",(pwd, username,))
		connection.commit()
		connection.close()
		return 1
